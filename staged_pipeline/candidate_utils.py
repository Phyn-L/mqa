...
TRIGGERS = {
    "comparison": {"compare", "than", "more", "less", "higher", "lower", "outnumber", "versus"},
    "cause_consequence": {"because", "therefore", "thus", "hence", "cause", "result", "lead", "due", "escalate"},
    "condition": {"if", "unless", "provided", "when", "only"},
    "temporal": {"before", "after", "during", "while", "later", "previously", "start", "end"},
...
def _deduplicate(items: list[dict[str, Any]], fields: tuple[str, ...]) -> list[dict[str, Any]]:
    ...
    for sentence_id, sentence in enumerate(sentences):
        ...
        for match in NUMBER_PATTERN.finditer(sentence.text):
            if not any(match.group() in item["text"] for item in numeric):
                numeric.append({
                    "text": match.group(), "label": "RULE_NUMERIC",
                    "sentence_id": sentence_id,
                })
                reasons.add("numeric_temporal")
        for token in sentence:
            ...
            for trigger_type, words in TRIGGERS.items():
                if lemma in words:
                    triggers.append({
                        "text": token.text, "type": trigger_type,
                        "sentence_id": sentence_id,
                    })
            ...
            objects = [child for child in token.children if child.dep_ in {"dobj", "obj", "iobj", "attr", "oprd", "dative"}]
            for prep in [child for child in token.children if child.dep_ in {"prep", "agent"}]:
        ...
        if reasons:
            priority_sentences.append({
                "sentence_id": sentence_id, "reasons": sorted(reasons),
                "evidence": sentence.text.strip(),
            })
    return {
        ...
        "relations": _deduplicate(relations, ("subject", "predicate", "object", "sentence_id")),
...
def compact_parser_candidates(candidates: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "entities": [[x["text"], x["label"], x["sentence_id"]] for x in candidates["entities"]],
        "numeric_temporal": [[x["text"], x["label"], x["sentence_id"]] for x in candidates["numeric_temporal"]],
        "triggers": [[x["text"], x["type"], x["sentence_id"]] for x in candidates["triggers"]],
        "relations": [[x["subject"], x["predicate"], x["object"], x["sentence_id"]] for x in candidates["relations"]],
        "priority_sentences": [[x["sentence_id"], x["reasons"]] for x in candidates["priority_sentences"]],
    }


def validate_reorganized(
    parsed: dict[str, Any] | None, context: str, seeds: dict[str, Any],
    min_coverage: float,
) -> tuple[list[str], dict[str, float]]:
    """Validate schema, evidence substrings, and seed coverage."""
    categories = ("events", "entities", "quantities", "triggers", "relations", "priority_sentences")
    ...
    for category in categories:
        ...
        for index, item in enumerate(items):
            ...
            if not isinstance(evidence, str) or not evidence.strip() or evidence not in context:
                errors.append(f"{category}[{index}].evidence is not a context substring")
            if category == "relations":
                ...
                if (
                    ...
                    errors.append(f"{category}[{index}] requires sentence_id and reasons")
            else:
    ...
    output_relations = [
        item for item in parsed.get("relations", [])
        if isinstance(item, dict)
    ]
    coverage["relations"] = (
        sum(
            1 for source in source_relations
            if any(
        ...
        if source_relations else 1.0
    )
    output_priority = {
        item.get("sentence_id") for item in parsed.get("priority_sentences", [])
        if isinstance(item, dict)
    }
    source_priority = seeds.get("priority_sentences", [])
    coverage["priority_sentences"] = (
        sum(1 for item in source_priority if item["sentence_id"] in output_priority) / len(source_priority)
        if source_priority else 1.0
    )
    for category, score in coverage.items():
        if score < min_coverage:
            errors.append(f"{category} coverage {score:.3f} is below {min_coverage:.3f}")
